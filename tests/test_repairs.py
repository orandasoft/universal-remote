...
def test_create_and_delete_missing_infrared_issue(hass: HomeAssistant) -> None:
    ...
    issue = issue_registry.async_get_issue(
        DOMAIN,
        f"{ISSUE_LINKED_INFRARED_EMITTER_MISSING}_living_room_tv",
    )
...
def test_delete_stale_missing_infrared_issues(hass: HomeAssistant) -> None:
    ...
    assert (
        issue_registry.async_get_issue(
            DOMAIN, f"{ISSUE_LINKED_INFRARED_EMITTER_MISSING}_keep"
        )
        is not None
    )
    assert (
        issue_registry.async_get_issue(
            DOMAIN, f"{ISSUE_LINKED_INFRARED_EMITTER_MISSING}_stale"
        )
        is None
    )
...
async def test_async_delete_stale_linked_infrared_emitter_missing_issues_keeps_configured_issue(
    hass: HomeAssistant,
) -> None:
